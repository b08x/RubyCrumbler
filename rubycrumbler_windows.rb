  ...
  class PipelineFeatures
    ...
    def tokenizer(language)
      Dir.glob(@projectdir + '/*.*').max_by(@filenumber) { |f| File.mtime(f) }.each do |file|
        ...
        doc.each do |token|
          count += 1
          row << token.text if token.text.strip != ''
        end
...
class CrumblerGUI
  ...
  def launch
    ...
    menu('Help') do
      ...
      menu_item('About') do
        on_clicked do
          window('About Crumbler', 700, 500, has_menubar = false) do
            on_closing do
              window.destroy
              1
            end
            ...
            vertical_box do
              ...
              button('Go to GitHub Repository') do
                stretchy false
                on_clicked do
                  `start https://github.com/joh-ga/Crumbler`
                end
              end
            end
          end.show
      ...
      menu_item('Documentation') do
        on_clicked do
          window('Documentation', 1500, 775, has_menubar = false) do
            on_closing do
              window.destroy
              1
            end
            ...
            vertical_box do
              area do
                text do
                  ...
                  string do
                    font family: 'Helvetica', size: 10, weight: :bold, italic: :normal, stretch: :normal
                    "Description of Features\n\n"
                  end
                  string("Please find below all the necessary information about the individual features.\n\n")
                  string do
                    font family: 'Helvetica', size: 9, weight: :bold, italic: :normal, stretch: :normal
                    underline :single
                    "Pre-Processing\n"
                  end
                  string do
